...
Let ${\tau}$ be a type among: `Integer`, `BigInteger`, `Float`, `Sym`, `Inf`, `NAN`, `Undefined`, `Bool`, `Str`, `SymStr`, and `Nil`. If ${a}$ is of type ${\tau}$ Then ${a ~~ \texttt{==} ~~ b}$ reduces to `true` if and only if (1) ${b}$ is also of type ${\tau}$ and (2) the value of ${b}$ is the value of ${a.}$ Otherwise:

1. If ${a}$ is a `BigFloat`, then ${a ~~ \texttt{==} ~~ b}$ if and only if ${b}$ is also a `BigFloat` and (1) the mantissa of ${a}$ is the mantissa of ${b,}$ and the exponent of ${a}$ is the exponent of ${b.}$ 
2. If ${a}$ is a `Complex`, then ${a ~~ \texttt{==} ~~ b}$ if and only if ${b}$ is a complex and (1) the real part of ${a}$ is the real part of ${b,}$ and the imaginary part of ${a}$ is the imaginary part of ${b.}$  
3. If ${a}$ is a `NumConst`, then ${a ~~ \texttt{==} ~~ b}$ if and only if ${b}$ is a `NumConst` and (1) the symbol of ${a}$ is the symbol of ${b,}$ and the value of ${a}$ is the value of ${b.}$ 
4. If ${a}$ is a `Frac`, then ${a ~~ \texttt{==} ~~ b}$ if and only if ${b}$ is a `Frac`, and, after ${a}$ and ${b}$ are reduced to their simplest forms, (1) the numerator of ${a}$ is the numerator of ${b,}$ and (2) the denominator of ${a}$ is the denominator of ${b.}$ 
4. If ${a}$ is a `BigFrac`, then ${a ~~ \texttt{==} ~~ b}$ if and only if ${b}$ is a `BigFrac`, and, after ${a}$ and ${b}$ are reduced to their simplest forms, (1) the numerator of ${a}$ is the numerator of ${b,}$ and (2) the denominator of ${a}$ is the denominator of ${b.}$ 
